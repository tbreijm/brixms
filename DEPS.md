...
| `wasmtime` | The WASM Driver host in brix-rt (issue #27). Component-model instantiation of `sdk/driver-wit/delta-abi.wit`'s `driver` world. |
| `wasmtime-wasi` | Companion to `wasmtime` for the same lane. `rustc --target wasm32-wasip2` (the component-model target the Driver SDK/example build against) links every guest against the standard WASI 0.2 worlds regardless of what the delta-ABI's own `capabilities` interface declares; the host must provide `wasmtime_wasi::add_to_linker_sync` or components fail to instantiate. Not a capability surface the delta ABI defines — purely what makes wasip2 binaries loadable at all. |
