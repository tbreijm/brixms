...
| `libc` | **Platform constants for non-blocking file handling only (`brix-lower`).** Provides portable POSIX open constants (`libc::O_NONBLOCK`) for Unix targets so input artifact shard opening avoids indefinite blocking on named pipes/FIFOs without hard-coding platform-specific integers (e.g. Linux vs macOS differences) or introducing unsafe workspace code. |
