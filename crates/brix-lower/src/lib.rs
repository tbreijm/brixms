...
pub enum LowerError {
    ...
    /// A call passes an argument whose type disagrees with the `fn`'s declared
    /// parameter type.
    ///
    /// Before this existed, `fn f(x: Str): Str = x` accepted `f(42)` and
    /// reported `@Proven` — the annotation was read for its name and its type
    /// discarded, so the arrow that got witnessed was not the arrow written.
    ParamTypeMismatch {
        function: String,
        param: String,
        declared: String,
...
fn config_ty_error(e: ConfigTyError) -> LowerError {
    match e {
        ConfigTyError::Unknown(name) => LowerError::UnknownDeclaredType(name),
        ConfigTyError::Arity {
            config,
            expected,
            found,
        ...
        ConfigTyError::Mutual(cycle) => LowerError::MutuallyRecursiveConfig {
...
fn check_declared_field_types(
    ...
    match e {
        ...
        ast::Expr::Call { func, args } => {
            ...
            // They were read only for their names, so `fn f(x: Str): Str = x`
            // accepted `f(42)` and published `@Proven` over it — the same
            // defect the `let` annotation path already refuses, in the
            // construct next door.
            let Some(callable) = ctx.fns.get(func) else {
                return Ok(());
            };
            if callable.params.len() != args.len() {
                return Ok(()); // arity is reported by lowering
            }

            for (param, arg) in callable.params.iter().zip(args.iter()) {
                let Some(declared) = &param.ty else { continue };
                let mut stack = Vec::new();
                let declared_ty = resolve_config_ty(declared, ctx.configs, &mut stack)
                    .map_err(config_ty_error)?;
