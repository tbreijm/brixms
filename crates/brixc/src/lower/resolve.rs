...
pub struct ProgramResolver {
    ...
    /// The exported relation capabilities of each *public* dependency symbol
    /// (issue #154), qualified name -> normalized [`ast::RelCaps`] (a bare `pub`
    /// relation resolves to `read` per errata 0003 Q2; `read` is implied by any
    /// `pub` per erratum 0004). This is the capability channel `RelVis` was
    /// missing on its way into `brixc`: the binary `is_public()` gate re-exports
    /// the symbol, and this map records *which* capabilities were granted so the
    /// `pub derive` orphan gate (BRX-LOW-0019) can tell an extensible head from
    /// a read-only one.
    export_caps: BTreeMap<QualIdent, ast::RelCaps>,
...
impl ProgramResolver {
    ...
    /// Record the exported relation capabilities of a public dependency symbol
    /// (issue #154). `caps` is already normalized by [`ast::Visibility::rel_caps`].
    pub fn with_export_caps(mut self, target: QualIdent, caps: ast::RelCaps) -> Self {
        self.export_caps.insert(target, caps);
        self
    }

    /// The exported capabilities of a (qualified) symbol, or `None` if it is not
    /// a known public dependency export. `None` therefore covers both
    /// package-private and root-local heads — neither is a foreign export the
    /// `pub derive` gate must consult.
    pub fn exported_caps(&self, target: &QualIdent) -> Option<ast::RelCaps> {
