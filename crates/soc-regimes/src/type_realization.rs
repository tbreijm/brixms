...
///
/// It differs from [`g_record_empty`] in one way that matters, and the gate
/// covers it. `{}` has a single instance; this generator ranges over every
/// user-declared sum type, and its leaf asserts something the host looked up —
/// that `variant` is declared by `sum_ty` with zero fields. **That lookup is
/// decidable from the leaf's own `src`**, because `Expr::Ctor` carries the sum
/// type inline, complete with its variant list. So the claim is self-contained:
/// a checker reading only `src` and `dst` can confirm it without trusting the
/// host, which is what ADR-0015 §8.5 asks for.
///
/// The precondition is enforced before emission — an unknown variant or a
/// nonzero arity is a `TypeError` and no leaf is produced —
/// and `zero_arity_intro_generators_are_faithful` pins both the positive shape
/// (`dst` is always exactly the `sum_ty` named in `src`) and the negative
/// (a bogus variant yields no derivation at all).
...
/// every generator — including the nineteen discharged below.
///
...
mod tests {
    ...
    fn zero_arity_intro_generators_are_faithful() {
        ...
        //     arity disagrees. This is what makes the claim self-contained:
        //     every leaf that exists is one whose story `src` already tells.
        for (label, expr) in [
            (
                "undeclared variant",
    ...
    fn claim_kind_typing_discharge_is_not_portable() {
        ...
        assert_eq!(
            ...
            19,
            "this test must cover exactly the nineteen typing-tight generators"
        );
