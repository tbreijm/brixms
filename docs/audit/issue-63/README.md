...
| ~~Realization route (outside kernels)~~ | ~~`type_realization.rs:type_check`~~ | ~~`Derived`, `SettlementReplay` of a recorded `Decomposition`~~ | **Removed** by [#262](https://github.com/tbreijm/brixms/issues/262) / [ADR-0018](../../../spec/adr/ADR-0018_Retire_The_Flat_Typing_Lane.md): its chain was padded to `[src, dst, dst, …]`, so the decomposition it recorded misstated its own intermediate configurations. No caller outside its own tests. |
| ~~Realization audit route (outside kernels)~~ | ~~`type_realization.rs:audited_type_check`~~ | ~~`Audited`, `SettlementReplay` of a replay-verified `Decomposition`~~ | **Removed** by [#262](https://github.com/tbreijm/brixms/issues/262) / ADR-0018: same padded chain, stamped `replay_verified` — a replay `soc_core::audit_step` rejects under sound generator semantics. `audit_step` is now the only caller of `Decomposition::replay_verified`. |
| Tree realization audit route (outside kernels) | `soc-regimes/src/type_realization.rs:audited_type_check_tree` | `Audited`, `Evidence::TreeDerivation` of a structure-verified `TreeDerivation` | **Resolved** by [#259](https://github.com/tbreijm/brixms/issues/259) / [ADR-0017](../../../spec/adr/ADR-0017_Tree_Realization_Support.md): publishes through `Judgement::publish` on an artifact `soc-regimes::tree_audit::audit_tree` earned — structure, endpoints, and leaf generator membership. ρ-membership remains deferred to ADR-0015 ⟨D-PRIM⟩. After #262 this is the **only** typing publisher. |
...
| A-3 | Authority risk | `soc-regimes/type_realization.rs:type_check` creates `Derived`; `audited_type_check` and `audited_type_check_tree` directly create `Audited` outside `soc-core`. | **Resolved.** All three were first routed through `Judgement::publish` against `ROUTES` (#228 / ADR-0016). [#259](https://github.com/tbreijm/brixms/issues/259) / [ADR-0017](../../../spec/adr/ADR-0017_Tree_Realization_Support.md) then gave `audited_type_check_tree` a checked `TreeDerivation` artifact and the leaf generator-membership check it lacked; [#262](https://github.com/tbreijm/brixms/issues/262) / [ADR-0018](../../../spec/adr/ADR-0018_Retire_The_Flat_Typing_Lane.md) **removed** `type_check` and `audited_type_check` outright — their padded chains misstated their own intermediate configurations, and nothing called them. **A-3 is now fully closed** by [ADR-0019](../../../spec/adr/ADR-0019_Verification_Tags_Are_Earned.md): a verified tag is the output of a checked transition, never a constructor input. `Decomposition::replay_verified` and `TreeDerivation::structure_verified` are **removed**; `verify_replay` and `verify_structure` perform their defining checks in the crate that owns the artifact, and both artifacts' fields are private, so the tag cannot be set by direct assignment either — a door §7.1 had not noticed. `compile_fail` gates enforce all five. What remains is narrower and is stated in ADR-0019 §6: the caller still supplies the `GeneratorSemantics`, so the predicate is guaranteed to have been *executed*, not *authenticated*, and a verified id does not name the semantics or registry it was verified against. |
