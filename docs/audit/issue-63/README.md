...
| A-3 | Authority risk | `soc-regimes/type_realization.rs:type_check` creates `Derived`; `audited_type_check` and `audited_type_check_tree` directly create `Audited` outside `soc-core`. | **Partly addressed** by #228 / ADR-0016: all three now publish through `Judgement::publish` against `ROUTES`, so their outcome/evidence pairings are checked. The residual — that `Decomposition::replay_verified` is itself an unchecked stamp, so `audited_type_check` tags a chain nothing replayed — is unchanged and still routes to #178/Track 2 (ADR-0016 §7.1). `audited_type_check_tree`'s support is worse than that and is written up separately: [#259](https://github.com/tbreijm/brixms/issues/259) / `spec/errata/0004-tree-realization-audited-support.md`. |
