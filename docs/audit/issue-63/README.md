...
| A-3 | Authority risk | `soc-regimes/type_realization.rs:type_check` creates `Derived`; `audited_type_check` and `audited_type_check_tree` directly create `Audited` outside `soc-core`. | **Partly addressed.** #228 / ADR-0016 routed all three through `Judgement::publish` against `ROUTES`, so their outcome/evidence pairings are checked. [#259](https://github.com/tbreijm/brixms/issues/259) / [ADR-0017](../../../spec/adr/ADR-0017_Tree_Realization_Support.md) then **resolved the tree route**: `audited_type_check_tree` now publishes on a checked `TreeDerivation` artifact earned by `soc-regimes::tree_audit::audit_tree`, which also adds the leaf generator-membership check the lane was missing. Two residuals remain, both open: `audited_type_check`'s padded chain ([#262](https://github.com/tbreijm/brixms/issues/262) / `spec/errata/0005-flat-path-padded-decomposition.md`), and the general fact that `Decomposition::replay_verified` is an unchecked stamp (ADR-0016 §7.1, #178/Track 2). |
