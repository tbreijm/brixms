...
**Landed 2026-08-03** in `saturate/driver.rs` and `saturate/bisimulation.rs`, gated by `tests/saturated_bisimulation.rs`. Two implementation notes worth recording. First, `check_saturated` requires **both** P1 and P6 declared, where §7.3 states only P1 normatively: the coinductive close needs `sat` to be a function of the `ObservableState`, and phase-dependent keying breaks that exactly as history-dependent enumeration does — the same pairing `sat_step` already applies when closing a lasso. Tightening toward fail-closed. Second, `SaturatedSystem` deliberately does **not** expose the journal: journals are permitted to differ between systems that agree behaviorally, so the comparison API is built so that comparing them is not expressible.
